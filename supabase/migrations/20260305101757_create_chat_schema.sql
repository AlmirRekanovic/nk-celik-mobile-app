/*
  ...
  2. Changes
    - Add `chat_nickname` column to members table for unique nicknames
    - Add unique constraint on chat_nickname
    
  3. Security
    - Enable RLS on `chat_messages` table
    - Add policy for authenticated members to read all messages
    - Add policy for authenticated members to create their own messages
    - Add policy for authenticated members to update/delete their own messages
    
  4. Indexes
    - Index on created_at for efficient message ordering
    - Index on member_id for efficient user message queries
*/

-- Add chat_nickname column to members table if it doesn't exist
DO $$
BEGIN
  IF NOT EXISTS (
...
CREATE POLICY "Members can read all messages"
  ...
  USING (
    ...
    EXISTS (
      SELECT 1 FROM members WHERE id = auth.uid()
    )
  );
...
-- Function to generate unique chat nickname
...
DECLARE
  ...
  counter int := 0;
BEGIN
  -- Create base nickname: firstname + lastname without spaces, lowercase
  base_nickname := lower(regexp_replace(p_first_name || p_last_name, '\s+', '', 'g'));
  final_nickname := base_nickname;
  
  -- Check if nickname exists, if so add 'celik' suffix
...
-- Update existing members to have chat nicknames
DO $$
DECLARE
...
BEGIN
  FOR member_record IN 
    ...
    FROM members 
    WHERE chat_nickname IS NULL
  LOOP
    new_nickname := generate_chat_nickname(member_record.first_name, member_record.last_name);
    UPDATE members 
    SET chat_nickname = new_nickname 
    WHERE id = member_record.id;
  END LOOP;
END $$;

-- Trigger to auto-generate chat_nickname on member insert
