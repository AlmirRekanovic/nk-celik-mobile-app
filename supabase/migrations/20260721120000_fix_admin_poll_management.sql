/*
  ...
  ## Problem
  A prior RLS hardening pass replaced the admin-aware INSERT/UPDATE/DELETE
  policies on `polls` with a single catch-all restricted to `service_role`.
  The mobile app uses the anon publishable key, so admins could not create,
  edit, or delete polls from the app — every write silently failed RLS
  and looked like a broken feature.

  ## Fix
  - Drop the service-role-only catch-all policy.
  - Re-introduce INSERT/UPDATE/DELETE policies that check the *current*
    member context (set by `set_member_context()` before each privileged
    write), not the row's `created_by`. Same pattern used by the chat admin
    policies.
  - Add an admin SELECT policy so admins can list inactive polls
    (`getAllPolls`); without it the admin dashboard only sees `is_active=true`.
  ...
  - The service role has implicit BYPASSRLS, so edge functions and the
    admin dashboard continue to work.
*/
