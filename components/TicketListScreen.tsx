...
import { Calendar, Trash2 } from '@/components/Icons';
import QRCode from 'react-native-qrcode-svg';

type Kind = 'match' | 'season';

interface Props {
  kind: Kind;
  title: string;
  emptySubtitle: string;
  HeaderIcon: ComponentType<{ size: number; color: string }>;
}

export default function TicketListScreen({ kind, title, emptySubtitle, HeaderIcon }: Props) {
  ...
  const loadTickets = async () => {
    ...
    try {
      ...
      const data = member.email
        ? await fetchTicketsByEmail(member.email, kind)
        : await fetchMemberTickets(member.id, kind);
  ...
  const handleDeleteTicket = (ticketId: string) => {
    Alert.alert('Obriši kartu', 'Da li ste sigurni da želite obrisati ovu kartu?', [
        ...
        { text: 'Otkaži', style: 'cancel' },
        {
          ...
          onPress: async () => {
            ...
            if (deletedCount > 0) {
              ...
              Alert.alert(
                'Uspjeh',
                `Obrisano ${deletedCount} ${deletedCount === 1 ? 'karta' : 'karata'}`
              );
            } else {
  ...
  const renderTicket = ({ item }: { item: Ticket }) => {
    ...
    return (
      ...
      >
        ...
        {!isExpanded && <Text style={styles.tapToExpand}>Klikni za prikaz QR koda</Text>}
      </TouchableOpacity>
    );
  };
    ...
    return (
      <View style={[styles.container, { backgroundColor }]}>
        ...
        <View style={styles.emptyContainer}>
          <HeaderIcon size={64} color="#9CA3AF" />
...
const styles = StyleSheet.create({
  ...
  headerDescription: { fontSize: 14, color: '#FEE2E2', flex: 1 },
  deleteAllButton: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  deleteAllButtonText: { fontSize: 12, fontWeight: '600', color: '#DC2626' },
  loadingContainer: {
    flex: 1,
  ...
  retryButtonText: { color: '#000000', fontSize: 16, fontWeight: '600' },
  ...
  ticketType: { fontSize: 18, fontWeight: '600', color: '#111827', marginBottom: 8 },
  ...
  qrLabel: { marginTop: 12, fontSize: 14, fontWeight: '600', color: '#DC2626', textAlign: 'center' },
  ticketDetails: {
    marginTop: 16,
    width: '100%',
    backgroundColor: '#F9FAFB',
  ...
  tapToExpand: { fontSize: 13, color: '#DC2626', textAlign: 'center', marginTop: 4 },
  emptyContainer: {
    flex: 1,
    justifyContent: 'center',
    alignItems: 'center',
